    ...
    <div className="space-y-4 pb-20" style=\{\{ backgroundColor: '#E9F4FF' \}\}>\
      ...
      <div className="grid grid-cols-3 gap-2 px-4 pt-4">\
        <div className="bg-white rounded-xl p-3 shadow-sm" style=\{\{ borderColor: '#A6FFC8', borderWidth: '1px' \}\}>\
          ...
          <p className="text-lg font-bold" style=\{\{ color: '#3C3F58' \}\}>\uc0\u8360 \{(totalIncome/1000).toFixed(0)\}k</p>\
        </div>\
        \
        <div className="bg-white rounded-xl p-3 shadow-sm" style=\{\{ borderColor: '#FD830D', borderWidth: '1px' \}\}>\
          ...
          <p className="text-lg font-bold" style=\{\{ color: '#3C3F58' \}\}>\uc0\u8360 \{(totalExpenses/1000).toFixed(0)\}k</p>\
        </div>\
        \
        <div className=\{`bg-white rounded-xl p-3 shadow-sm`\} style=\{\{ borderColor: balance >= 0 ? '#4C51C6' : '#FD830D', borderWidth: '1px' \}\}>\
          ...
          <p className=\{`text-lg font-bold`\} style=\{\{ color: '#3C3F58' \}\}>\
            \uc0\u8360 \{(Math.abs(balance)/1000).toFixed(0)\}k\
          </p>\
        </div>\
      </div>\
\
              ...
              <div key=\{transaction.id\} className="p-4 flex items-center justify-between">\
                ...
                <div className="text-right">\
                  <p className=\{`font-semibold text-sm`\} style=\{\{ color: transaction.type === 'income' ? '#A6FFC8' : '#FD830D' \}\}>\
                    \{transaction.type === 'income' ? '+' : '-'\}\uc0\u8360 \{(transaction.amount/1000).toFixed(1)\}k\
                  </p>\
      ...
      <div className="bg-white rounded-xl shadow-sm divide-y divide-gray-50">\
        \{transactions.map(transaction => (\
          <div key=\{transaction.id\} className="p-4 flex items-center justify-between">\
            ...
            <div className="text-right">\
              <p className=\{`font-semibold text-sm`\} style=\{\{ color: transaction.type === 'income' ? '#A6FFC8' : '#FD830D' \}\}>\
                \{transaction.type === 'income' ? '+' : '-'\}\uc0\u8360 \{transaction.amount.toLocaleString()\}\
              </p>\
              <p className="text-xs text-gray-500">\{new Date(transaction.date).toLocaleDateString()\}</p>\
            </div>\
          </div>\
            ...
            return (\
              <div key=\{category\} className="space-y-2">\
                <div className="flex justify-between items-center">\
                  ...
                  <span className="text-sm font-semibold" style=\{\{ color: '#3C3F58' \}\}>\uc0\u8360 \{amount.toLocaleString()\}</span>\
                </div>\
                ...
                \{budget > 0 && (\
                  <p className="text-xs text-gray-500">\{Math.round(percentage)\}% of \uc0\u8360 \{budget.toLocaleString()\} budget</p>\
                )\}\
              </div>\
            );\
          \})\}\
          ...
          <div key=\{goal.id\} className="bg-white rounded-xl p-4 shadow-sm">\
            ...
            <div className="space-y-2">\
              <div className="flex justify-between text-sm">\
                ...
                <span className="font-medium" style=\{\{ color: '#3C3F58' \}\}>\uc0\u8360 \{goal.current.toLocaleString()\} / \u8360 \{goal.target.toLocaleString()\}</span>\
              </div>\
              ...
              <p className="text-xs text-gray-500">\uc0\u8360 \{remaining.toLocaleString()\} remaining</p>\
            </div>\
          </div>\
        );\
      \})\}\
...
\
              <div>\
                <label className="block text-sm font-medium mb-2" style=\{\{ color: '#3C3F58' \}\}>Amount (\uc0\u8360 )</label>\
                <input\
                  type="number"\
